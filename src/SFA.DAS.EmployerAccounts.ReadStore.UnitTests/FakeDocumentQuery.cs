...
namespace SFA.DAS.EmployerAccounts.ReadStore.UnitTests
{
    public class FakeDocumentQuery<T> : IDocumentQuery<T>, IOrderedQueryable<T>
    {
        ...
        public Task<FeedResponse<TResult>> ExecuteNextAsync<TResult>(CancellationToken token)
        {
            return Task.FromResult(new FeedResponse<TResult>(this.Cast<TResult>()));
        }

        public Task<FeedResponse<dynamic>> ExecuteNextAsync(CancellationToken token)
        {
