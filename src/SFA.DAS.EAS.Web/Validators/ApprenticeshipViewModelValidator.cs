...
namespace SFA.DAS.EAS.Web.Validators
{
    public class ApprenticeshipViewModelValidator : AbstractValidator<ApprenticeshipViewModel>
    {
        public ApprenticeshipViewModelValidator()
        {
            ...
            RuleFor(x => x.StartMonth).InclusiveBetween(1, 12).WithMessage("Enter a valid month");
            RuleFor(x => x.StartYear).InclusiveBetween(currentYear, 9999).WithMessage("Enter a valid year");
            RuleFor(x => x.EndMonth).InclusiveBetween(1, 12).WithMessage("Enter a valid month");
            RuleFor(x => x.EndYear).InclusiveBetween(currentYear, 9999).WithMessage("Enter a valid year");
        }
    }
}