...
namespace SFA.DAS.EAS.Web.Controllers
{
    ...
    public class EmployerAccountPayeController : BaseController
    {
        ...
        {
            var model = await _employerAccountPayeOrchestrator.Get(hashedAccountId, GetUserId());
        ...
        {
            var model = await _employerAccountPayeOrchestrator.GetNextStepsViewModel(Guid.Parse(OwinWrapper.GetClaimValue(ControllerConstants.UserExternalIdClaimKeyName)),hashedAccountId);
        ...
        public async Task<ActionResult> Details(string hashedAccountId, string empRef)
        {
            ...
            var response = await _employerAccountPayeOrchestrator.GetPayeDetails(empRef, hashedAccountId, GetUserId());
        ...
        public async Task<ActionResult> ConfirmPayeScheme(string hashedAccountId, AddNewPayeSchemeViewModel model)
        {
            var result = await _employerAccountPayeOrchestrator.AddPayeSchemeToAccount(model,
                Guid.Parse(OwinWrapper.GetClaimValue(ControllerConstants.UserExternalIdClaimKeyName)));
        ...
        public async Task<ActionResult> Remove(string hashedAccountId, string empRef)
        {
            var model = await _employerAccountPayeOrchestrator.GetRemovePayeSchemeModel(new RemovePayeSchemeViewModel
            {
                ...
                ExternalUserId = Guid.Parse(OwinWrapper.GetClaimValue(ControllerConstants.UserExternalIdClaimKeyName))
            });
        ...
        {
            model.ExternalUserId = Guid.Parse(OwinWrapper.GetClaimValue(ControllerConstants.UserExternalIdClaimKeyName));
