CREATE PROCEDURE [employer_transactions].[GetTransactionDetail_ByDateRange]
	@accountId bigint,
	@fromDate datetime,
	@toDate datetime
...
select 
    ...
    tl.TransactionType
from [employer_transactions].TransactionLine tl
inner join [employer_transactions].LevyDeclarationTopup ldt on ldt.SubmissionId = tl.SubmissionId
OUTER APPLY
(
	SELECT TOP 1 Amount
...
select 
    ...
    tl.TransactionType
from [employer_transactions].TransactionLine tl
inner join [employer_transactions].Payment p on p.PeriodEnd = tl.PeriodEnd and p.AccountId = tl.AccountId
inner join [employer_transactions].PaymentMetaData meta on p.PaymentMetaDataId = meta.Id
where   tl.TransactionDate >= @fromDate AND 
        tl.TransactionDate <= @toDate AND 
        tl.AccountId = @accountId