    ...
    public static class StringExtensions
    {
        ...
        public static bool IsEquivalent(this string s1, string s2, StringEquivalenceOptions options)
        {
            ...
            while (idx1 < s1.Length && idx2 < s2.Length)
            {
                ...
                if ((!caseInsensitive && s1[idx1] != s2[idx2]) || 
                   (caseInsensitive && char.ToUpperInvariant(s1[idx1]) != char.ToUpperInvariant(s2[idx2])))
                {
                    return false;
                }
        ...
        /// </summary>
        private static bool SkipToNextNonSpace(string s, ref int idx)
        {
