    ...
    public class EnglishFractionRepository : BaseRepository, IEnglishFractionRepository
    {
        ...
        public async Task<DateTime> GetLastUpdateDate()
        {
            var result = await WithConnection(async c => await c.QueryAsync<DateTime>(
                sql: "SELECT Top(1) DateCalculated FROM [employer_transactions].[EnglishFractionCalculationDate] ORDER BY DateCalculated DESC;",
        ...
        public async Task SetLastUpdateDate(DateTime dateUpdated)
        {
            await WithConnection(async c =>
            {
                ...
                return await c.ExecuteAsync(
                    sql: "INSERT INTO [employer_transactions].[EnglishFractionCalculationDate] (DateCalculated) VALUES (@dateCalculated);",
                    param: parameters,
                    commandType: CommandType.Text);
            });
        }
        ...
        public async Task<DasEnglishFraction> GetEmployerFraction(DateTime dateCalculated, string employerReference)
        {
            var result = await WithConnection(async c =>
            {
                ...
                return await c.QueryAsync<DasEnglishFraction>(
                    sql: "SELECT * FROM [employer_transactions].[EnglishFraction] WHERE EmpRef = @empRef AND DateCalculated = @dateCalculated;",
                    param: parameters,
                    commandType: CommandType.Text);
            });
        ...
        public async Task<DasEnglishFraction> GetCurrentFractionForScheme(string employerReference)
        {
            var result = await WithConnection(async c =>
            {
                ...
                return await c.QueryAsync<DasEnglishFraction>(
                    sql: "SELECT top 1 * FROM [employer_transactions].[EnglishFraction] WHERE EmpRef = @empRef Order by DateCalculated desc;",
                    param: parameters,
                    commandType: CommandType.Text);
            });
        ...
        public async Task<IEnumerable<DasEnglishFraction>> GetAllEmployerFractions(string employerReference)
        {
            var result = await WithConnection(async c =>
            {
                ...
                return await c.QueryAsync<DasEnglishFraction>(
                    sql: "SELECT * FROM [employer_transactions].[EnglishFraction] WHERE EmpRef = @empRef ORDER BY DateCalculated desc;",
                    param: parameters,
                    commandType: CommandType.Text);
            });
        ...
        public async Task CreateEmployerFraction(DasEnglishFraction fractions, string employerReference)
        {
            await WithConnection(async c =>
            {
                ...
                return await c.ExecuteAsync(
                    sql: "INSERT INTO [employer_transactions].[EnglishFraction] (EmpRef, DateCalculated, Amount) VALUES (@empRef, @dateCalculated, @amount);",
                    param: parameters,
                    commandType: CommandType.Text);
            });
        }
