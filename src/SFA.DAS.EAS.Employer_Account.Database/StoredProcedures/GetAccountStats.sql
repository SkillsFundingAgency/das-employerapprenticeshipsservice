...
AS
	...
	SELECT
		...
		(SELECT COUNT(1) FROM [employer_account].[Invitation] WHERE AccountId = @accountId AND Status != 4) AS TeamMembersInvited,
		(SELECT COUNT(1) FROM [employer_account].[AccountLegalEntity] WHERE AccountId = @accountId AND SignedAgreementId IS NOT NULL) AS SignedAgreementCount