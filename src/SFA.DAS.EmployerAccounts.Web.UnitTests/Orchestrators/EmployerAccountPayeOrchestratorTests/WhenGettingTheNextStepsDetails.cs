...
namespace SFA.DAS.EmployerAccounts.Web.UnitTests.Orchestrators.EmployerAccountPayeOrchestratorTests
{
    public class WhenGettingTheNextStepsDetails
    {
        ...
        public void Arrange()
        {
            ...
            _mediator.Setup(x => x.SendAsync(It.IsAny<GetTeamMemberQuery>())).ReturnsAsync(new GetTeamMemberResponse {User = new MembershipView {ShowWizard = true, Role = (short)Role.Owner}});
        ...
        {
            //Arrange
            _mediator.Setup(x => x.SendAsync(It.IsAny<GetTeamMemberQuery>())).ReturnsAsync(new GetTeamMemberResponse { User = new MembershipView { ShowWizard = true, Role = (short)Role.Transactor } });
            var expectedUserId = "AFGV1234";
            var expectedAccountId = "789GBT";
