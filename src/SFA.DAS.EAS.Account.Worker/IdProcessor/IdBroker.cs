...
namespace SFA.DAS.EAS.Account.Worker.IdProcessor
{
    public class IdBroker : IIdBroker
    {
        ...
        {
            var lastSuccessfullyProcessedId = await _idCheckpoint.GetLastCheckpointAsync(idProvider);

            var result = new ProcessingInfo
            {
                BatchSize = _idProcessorConfiguration.BatchSize,
                StartTime = DateTime.UtcNow,
                LastProcessedId = lastSuccessfullyProcessedId,
        ...
        {
            var ids = await idProvider.GetIdsAsync(processingInfo.LastProcessedId, processingInfo.BatchSize, processingContext);
            ...
            foreach (var id in ids.Where(i => i > processingInfo.LastProcessedId).Take(processingInfo.BatchSize))
            {
                AssertIdsInCorrectOrder(id, ref previousId, processingInfo);

                if (!await ProcessNextIdAsync(processingInfo, id, processor, processingContext))
                {
                    break;
                }
        ...
        private async Task<bool> ProcessNextIdAsync(ProcessingInfo processingInfo, long id, IProcessor processor, ProcessingContext processingContext)
        {
            bool shouldContinue;
            try
            {
