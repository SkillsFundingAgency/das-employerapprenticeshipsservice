...
namespace SFA.DAS.EAS.Portal.Client.Application.Queries
{
    internal class GetAccountQuery
    {
        ...
        public async Task<Account> Get(long accountId, bool getRecruit, CancellationToken cancellationToken = default)
        {
            ...
            if (getRecruit)
            {
                var vacanciesSummary = await _dasRecruitService.GetVacanciesSummary(document.AccountId);
                document.Account.Vacancies = vacanciesSummary.Vacancies.Select(x => _dasRecruitService.MapToVacancy(x)).ToList();
            }
