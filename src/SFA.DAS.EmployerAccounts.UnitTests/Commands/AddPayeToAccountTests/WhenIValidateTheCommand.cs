...
namespace SFA.DAS.EmployerAccounts.UnitTests.Commands.AddPayeToAccountTests
{
    public class WhenIValidateTheCommand
    {
        ...
        public void Arrange()
        {
            ...
            _membershiprepository.Setup(x => x.GetCaller(It.IsAny<string>(), ExpectedOwnerUserId)).ReturnsAsync(new MembershipView {Role = (short)Role.Owner});
            _membershiprepository.Setup(x => x.GetCaller(It.IsAny<string>(), ExpectedNonOwnerUserId)).ReturnsAsync(new MembershipView {Role = (short)Role.Viewer});
