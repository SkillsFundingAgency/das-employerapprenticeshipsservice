...
namespace SFA.DAS.EAS.MessageHandlers.Worker.EventHandlers
{
    ...
    public class RejectedTransferConnectionInvitationEventHandler : MessageProcessor<RejectedTransferConnectionInvitationEvent>
    {
        ...
        public RejectedTransferConnectionInvitationEventHandler(IMessageSubscriberFactory subscriberFactory, ILog log, Lazy<EmployerAccountDbContext> dbContext, INotificationsApi notificationsApi, EmployerApprenticeshipsServiceConfiguration employerApprenticeshipsServiceConfiguration) : base(subscriberFactory, log)
        {
            _db = dbContext;
        ...
        protected override Task OnErrorAsync(IMessage<RejectedTransferConnectionInvitationEvent> message, Exception ex)
        {
            ...
            {
                _log.Error(ex, $"Could not process RejectedTransferConnectionInvitationEvent message NULL content");
