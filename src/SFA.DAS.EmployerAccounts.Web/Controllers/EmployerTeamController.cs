    ...
    public class EmployerTeamController : BaseController
    {
        ...
        public async Task<ActionResult> Index(string hashedAccountId, string reservationId)
        {
            ...
            if (FeatureToggles.Features.HomePage.Enabled || !(response.Data.HasPaye))
            {
                ...
                response.Data.ApprenticeshipAdded = response.Data.AccountViewModel?.Organisations?.FirstOrDefault() != null && response.Data.AccountViewModel?.Organisations?.FirstOrDefault().Cohorts?.FirstOrDefault() != null && response.Data.AccountViewModel?.Organisations?.FirstOrDefault().Cohorts?.FirstOrDefault().Apprenticeships?.Count > 0;
